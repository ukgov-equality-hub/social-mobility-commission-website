...
resource "aws_ses_domain_identity" "ses_domain_identity" {
  domain = "mail.${var.dns_record_subdomain_including_dot__email_domain}${data.aws_route53_zone.route_53_zone_for_our_domain.name}"
}
...
resource "aws_route53_record" "route53_amazonses_dkim_record" {
  ...
  name    = "${aws_ses_domain_dkim.ses_domain_dkim.dkim_tokens[count.index]}._domainkey.mail.${var.dns_record_subdomain_including_dot__email_domain}${data.aws_route53_zone.route_53_zone_for_our_domain.name}"
  type    = "CNAME"
  ttl     = "300"
...
resource "aws_ses_domain_mail_from" "ses_domain_mail_from" {
  domain           = aws_ses_domain_identity.ses_domain_identity.domain
  mail_from_domain = "contact-us.mail.${var.dns_record_subdomain_including_dot__email_domain}${data.aws_route53_zone.route_53_zone_for_our_domain.name}"
}
