...
data "aws_iam_policy_document" "s3_bucket_policy__uploads" {
  statement {
    principals {
      type = "Service"
    ...
    resources = [
      "${aws_s3_bucket.s3_bucket__uploads.arn}/*"  // The files within the bucket
    ]
    condition {
      test = "StringEquals"
...
resource "aws_s3_bucket_policy" "static_website_s3_bucket_policy" {
  bucket = aws_s3_bucket.s3_bucket__uploads.id
  policy = data.aws_iam_policy_document.s3_bucket_policy__uploads.json
}
