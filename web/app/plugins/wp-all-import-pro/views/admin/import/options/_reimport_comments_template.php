							...
							<td>
								...
								<div>
									<label>
										...
										<h4 style="margin: 0; display: inline-block; font-size: 14px;"><?php printf(__('Create new %s for each record in this import file', 'wp-all-import-pro'), $custom_type->labels->name); ?></h4>
									</label>
								</div>
								<div class="wpallimport-wizard-description">
									<?php printf(__('First run creates new %s. Re-running this import will update those same %s using the Unique Identifier.', 'wp-all-import-pro'), $custom_type->labels->name, $custom_type->labels->name); ?>
								</div>
								...
								<div class="wpallimport-wizard-section-spacing">
									<label>
										...
										<h4 style="margin: 0; display: inline-block; font-size: 14px;"><?php printf(__('Attempt to match to existing %s before creating new ones', 'wp-all-import-pro'), $custom_type->labels->name); ?></h4>
									</label>
								</div>
								<div class="wpallimport-wizard-description">
									<?php printf(__('Records in this import file will be matched with %s on your site based on...', 'wp-all-import-pro'), $custom_type->labels->name); ?>
								</div>
								<div class="switcher-target-wizard_type_matching" <?php if ($post['wizard_type'] != 'matching') { ?> style="display: none;" <?php } ?>>
									<div class="wpallimport-wizard-type-options">
										...
										<label for="duplicate_indicator_pid"><?php _e('Comment ID', 'wp-all-import-pro' )?></label><br>
								...
								<div class="input" style="margin-bottom:15px; position:relative;">
									...
									<div style="margin-left: -4px;">
										<div class="input">
											...
											<div class="input">
												<input type="radio" id="duplicate_indicator_pid" class="switcher" name="duplicate_indicator" value="pid" <?php echo 'pid' == $post['duplicate_indicator'] ? 'checked="checked"': '' ?>/>
												<label for="duplicate_indicator_pid"><?php _e('Comment ID', 'wp-all-import-pro' )?></label>
											</div>
